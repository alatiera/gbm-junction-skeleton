...
build *ARGS:
    ...
    bst --strict build oci/placeholder/image.bst
    bst artifact checkout --tar - oci/placeholder/image.bst | run0 podman load
...
generate-bootable-image $base_dir=base_dir:
    ...
    if [ ! -e "${base_dir}/bootable.raw" ] ; then
        fallocate -l 25G "${base_dir}/bootable.raw"
    fi
...
run-qemu-vm $base_dir=base_dir:
    ...
    qemu-system-x86_64 \
        ...
        -drive "if=none,id=boot-disk,file=${base_dir}/bootable.raw,media=disk,format=raw,discard=on" \
